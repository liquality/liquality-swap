...
async function findClaimSwapTransaction (party, blockNumber, dispatch, getState) {
  ...
  const valueInUnit = cryptoassets[assets[oppositeParty].currency].currencyToUnit(assets[oppositeParty].value)
  const swapParams = {
    value: valueInUnit,
    recipientAddress,
    refundAddress,
...
async function findRefundSwapTransaction (party, blockNumber, dispatch, getState) {
  ...
  const valueInUnit = cryptoassets[assets[party].currency].currencyToUnit(assets[party].value)
  const swapParams = {
    value: valueInUnit,
